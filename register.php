...
if (isset($_REQUEST["subdomain"])) {
  ...
  if (!$doms) {
    ...
    echo "
Script Not Ready, but I'm close to wildcard cpanel subdomain creation<br>
<form method='post'>
  Website Name:<input name='subdomain'><br>
  <!--Domain:<input name='domain'><br>-->
  Username:<input name='cpaneluser'><br>
  Password:<input name='cpanelpass'><br>
 <!-- cPanel Skin:<input name='cpanelskin'><br>-->
  <input type='submit' value='Test Subdomain Creation' style='border:1px solid black'>
</form>";
    die();
  }
}
...
foreach($doms as $dom) {
  ...
  // http://[bizsmarty.com]:2082/frontend/x/subdomain/doadddomain.html?domain=[subdomain here]&rootdomain=[domain here]
